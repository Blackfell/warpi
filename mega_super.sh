...
function installKismet	{
	...
	cd kismet && sudo git fetch --all --tags --prune
	sudo git checkout tags/kismet-2021-08-R1 -b master	# We're gonna build 2021-08-R1 for now
	sudo ./configure
	if sudo make -j$(nproc) && sudo make suidinstall
	then
	       	cd $SAVED_WD
       		return -1	       
	else
